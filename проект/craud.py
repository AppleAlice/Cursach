...
class sql():
  ...
  def update(self):
    ...
    con.commit()
    con.close()
helper = """Здесь краткая инструкция по использованию программы
-h                  Вызов данной справки
-i <имя> <дата>     Внесение данных в таблицу, дата в формате: dd-mm-yyyy Пример: ./craud.py -i Vasya 21-01-2005
-s <имя>            Поиск данных по столбцу "Имя". Пример: ./craud.py -s Vasya  - Покажет все значение столбцов с именем "Vasya"
...
-e <id> <столбец> <значение> Для редактирования строки, укажите её "id", столбец(user, или data) который вы хотите отредактировать и его новое значение
                    Пример: ./craud.py -e 2 user Romka
"""
if len(sys.argv) == 1:
  print(helper)
if len(sys.argv) == 2:
  if sys.argv[1] == "-h":
    print("""Здесь краткая инструкция по использованию программы
-h              Вызов данной справки
-i имя дата     Внесение данных в таблицу, дата в формате: dd-mm-yyyy Пример: ./craud.py -i Vasya 21-01-2005
""")
if len(sys.argv) == 3:
